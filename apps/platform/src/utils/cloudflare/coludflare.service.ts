...
export class CloudflareService {
  ...
  constructor(
    ...
    if (
      cloudflareConfig.apiKey === undefined ||
      cloudflareConfig.accountId === undefined
    ) {
  ...
  public async createTunnel(tunnelName: string): Promise<any> {
    ...
    const res = await this.httpService.axiosRef.post<any, TunnelResponse>(
      url,
      data,
      { headers },
    );
    ...
    if (!res.success) {
      this.logger.error(
        `Failed to create Cloudflare tunnel: ${JSON.stringify(res.errors)}`,
      );
      throw new Exception(
  ...
  public async getTunnel(tunnelId: string): Promise<any> {
    ...
    const res = await this.httpService.axiosRef.get<any, TunnelResponse>(url, {
      headers,
    });

    if (!res.success) {
      this.logger.error(
        `Failed to get Cloudflare tunnel: ${JSON.stringify(res.errors)}`,
      );
      throw new Exception(
        Exception.GENERAL_SERVER_ERROR,
        'Failed to get Cloudflare tunnel',
      );
    }
  ...
  public async updateTunnel(
    ...
    const res = await this.httpService.axiosRef.put<any, TunnelConfigResponse>(
      url,
      data,
      { headers },
    );
    ...
    if (!res.success) {
      this.logger.error(
        `Failed to update Cloudflare tunnel: ${JSON.stringify(res.errors)}`,
      );
      throw new Exception(
        Exception.GENERAL_SERVER_ERROR,
        'Failed to update Cloudflare tunnel',
      );
    }
