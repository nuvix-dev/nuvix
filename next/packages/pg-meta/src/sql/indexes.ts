...
SELECT
  ...
  to_jsonb(string_to_array(idx.indkey::text, ' ')::smallint[]) AS key_attributes,
  to_jsonb(string_to_array(idx.indcollation::text, ' ')::integer[]) AS collation,
  to_jsonb(string_to_array(idx.indclass::text, ' ')::integer[]) AS class,
  to_jsonb(string_to_array(idx.indoption::text, ' ')::smallint[]) AS options,
...
FROM
  ...
  JOIN pg_attribute a ON a.attrelid = c.oid AND a.attnum = ANY(string_to_array(idx.indkey::text, ' ')::smallint[])
