...
function required(name: string, fallback?: string): string {
	...
	if (value === undefined || value === "") {
		throw new Error(
			`Missing required environment variable: ${name}. See docs/ENV.md.`,
		);
	}
	return value;
}
...
function readEnv(): NuvixEnv {
	...
	if (value !== "development" && value !== "production" && value !== "test") {
		throw new Error(
			`NUVIX_ENV must be development | production | test, got: ${value}`,
		);
	}
	return value;
}
...
export const config = {
	...
	platform: {
		...
		get port(): number {
			return parsePort(
				"NUVIX_PLATFORM_PORT",
				required("NUVIX_PLATFORM_PORT", "4100"),
			);
		},

		/** `sqlite` (default — zero-infra local dev) or `postgres`. */
		get dbDriver(): PlatformDbDriver {
			return parsePlatformDbDriver(
				Bun.env.NUVIX_PLATFORM_DB_DRIVER ?? "sqlite",
			);
		},
...
function parsePlatformDbDriver(raw: string): PlatformDbDriver {
	if (raw !== "sqlite" && raw !== "postgres") {
		throw new Error(
			`NUVIX_PLATFORM_DB_DRIVER must be sqlite | postgres, got: ${raw}`,
		);
	}
	return raw;
}
