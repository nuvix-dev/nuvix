...
import { createEncryptFilter, jsonFilter, registerCoreDbFilters } from './filters'

const document = new Doc({})

describe('jsonFilter', () => {
  test('round-trips a plain object through encode/decode', () => {
    const encoded = jsonFilter.encode({ host: 'db', port: 5432 }, document, undefined as never)
    expect(typeof encoded).toBe('string')
    expect(jsonFilter.decode(encoded as string, document, undefined as never)).toEqual({
      host: 'db',
      port: 5432,
    })
  })
  ...
  test('encodes null/undefined to null', () => {
    ...
    expect(jsonFilter.encode(undefined, document, undefined as never)).toBeNull()
  })
...
describe('createEncryptFilter', () => {
  ...
  test('composes with jsonFilter to encrypt structured data (json → encrypt order)', async () => {
    ...
    expect(jsonFilter.decode(decrypted as string, document, undefined as never)).toEqual({
      password: 'p@ss',
    })
  })

  test('non-string encode input becomes null', async () => {
    expect(await filter.encode(undefined, document, undefined as never)).toBeNull()
  })
})
