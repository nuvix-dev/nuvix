...
describe.skipIf(!hasDocker)('DockerTenantProvisioner (live nuvix/postgres:18.1)', () => {
  ...
  test('deprovision keeps the volume by default, purge removes it', async () => {
    ...
    const keptVolume = await Bun.spawn(['docker', 'volume', 'inspect', handle.volumeName], {
      stdout: 'ignore',
      stderr: 'ignore',
    }).exited
    ...
    await provisioner.deprovision(handle, { purge: true })
    const purgedVolume = await Bun.spawn(['docker', 'volume', 'inspect', handle.volumeName], {
      stdout: 'ignore',
      stderr: 'ignore',
    }).exited
