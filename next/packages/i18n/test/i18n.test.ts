...
describe("Translator", () => {
	...
	test("malformed ICU throws at format time", async () => {
		const loader = new TranslationLoader(
			new URL("./fixtures/broken", import.meta.url).pathname,
		);
...
describe("locale resolution", () => {
	...
	test("parses accept-language with q-values", () => {
		expect(parseAcceptLanguage("fr-CH, fr;q=0.9, en;q=0.8, *;q=0")).toEqual([
			"fr-ch",
			"fr",
			"en",
		]);
	...
	test("resolution chain: header > user > accept-language > default", () => {
		...
		expect(
			resolveLocale({ ...base, headerLocale: null, userLocale: "fr" }),
		).toBe("fr");
		expect(
			resolveLocale({
				...base,
		...
		expect(
			resolveLocale({ ...base, headerLocale: "de", userLocale: "fr" }),
		).toBe("de");
