...
const ASSETS_DIR = new URL("../../../../assets/avatars/", import.meta.url)
	.pathname;
const FONT_FILE = new URL(
	"../../../../assets/fonts/Varela-Regular.ttf",
	import.meta.url,
).pathname;
...
const clampDim = (v: number | undefined, fallback: number) => {
	...
	return Number.isFinite(n)
		? Math.min(MAX_DIM, Math.max(1, Math.trunc(n)))
		: fallback;
};
...
async function loadSet(dir: string): Promise<Set<string>> {
	...
	return new Set(
		entries.map((f) => path.basename(f, path.extname(f)).toLowerCase()),
	);
}
...
export function createAvatarService() {
  ...
  <text x="50%" y="50%" dominant-baseline="central" text-anchor="middle" fill="#ffffff"
	...
	async function initials(opts: {
		...
		const background =
			hex && (hex.length === 3 || hex.length === 6)
				? `#${hex}`
				: hslFromName(name);

		const svg = initialsSVG(
			getInitials(name),
			width,
			height,
			background,
	...
	async function favicon(url: string): Promise<AvatarResult> {
		...
		try {
			...
			const body = await new Blob([bytes])
				.image()
