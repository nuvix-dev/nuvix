...
export async function verifySessionSecret(
  ...
  const sessionDoc = await session.findOne('sessions', [Query.equal('secretHash', [secretHash])])
  ...
  if (expire) {
    const expireTime = new Date(expire instanceof Date ? expire : String(expire)).getTime()
