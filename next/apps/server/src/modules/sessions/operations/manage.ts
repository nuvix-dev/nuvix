...
export async function getSessionOrThrow(session: Session, sessionId: string): Promise<SessionsDoc> {
...
export async function listUserSessions(session: Session, userId: string): Promise<SessionsDoc[]> {
  return session.find('sessions', [Query.equal('userId', [userId])])
}

export async function deleteSession(session: Session, sessionId: string): Promise<void> {
...
export async function deleteAllUserSessions(
  ...
  await Promise.all(toDelete.map((s) => session.deleteDocument('sessions', s.getId())))
}
