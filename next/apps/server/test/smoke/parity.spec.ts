...
async function compareWithV1(case_: ParityCase): Promise<V1Comparison> {
	...
	try {
		...
		const [v2Sig, v1Sig] = await Promise.all([
			signature(v2Res),
			signature(v1Res),
		]);
		const diffs: string[] = [];
		if (v2Sig.status !== v1Sig.status)
			diffs.push(`status ${v2Sig.status} vs ${v1Sig.status}`);
		if (v2Sig.contentTypePrefix !== v1Sig.contentTypePrefix) {
			diffs.push(
				`content-type ${v2Sig.contentTypePrefix} vs ${v1Sig.contentTypePrefix}`,
			);
		}
		if (
			v2Sig.jsonKeys &&
			v1Sig.jsonKeys &&
			v2Sig.jsonKeys.join() !== v1Sig.jsonKeys.join()
		) {
			diffs.push(`json keys [${v2Sig.jsonKeys}] vs [${v1Sig.jsonKeys}]`);
		}
		if ((v2Sig.cacheControl ?? "") !== (v1Sig.cacheControl ?? "")) {
			diffs.push(
				`cache-control ${v2Sig.cacheControl} vs ${v1Sig.cacheControl}`,
			);
		}
...
afterAll(async () => {
	...
	console.log(
		`── ${matches}/${v1Results.length} normalized behaviors match ──\n`,
	);
});
...
describe("smoke: real-app behavioral parity", () => {
	for (const c of cases) {
		test(c.name, async () => {
			...
			if (c.contentType)
				expect(res.headers.get("content-type")!.startsWith(c.contentType)).toBe(
					true,
				);
			if (c.cacheControl)
				expect(res.headers.get("cache-control")).toBe(c.cacheControl);
			if (c.contentDisposition) {
				expect(
					res.headers
						.get("content-disposition")!
						.startsWith(c.contentDisposition),
				).toBe(true);
			}
			...
			if (!isJson) {
				...
				if (c.jsonKeys)
					expect(Object.keys(json).sort()).toEqual([...c.jsonKeys].sort());
				...
				if (c.envelope?.totalMin !== undefined) {
					expect((json.meta as { total: number }).total).toBeGreaterThanOrEqual(
						c.envelope.totalMin,
					);
				}
