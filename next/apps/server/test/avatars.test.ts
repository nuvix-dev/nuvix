...
describe('avatar routes', () => {
  ...
  test('GET /avatars/flags/:code → image/png', async () => {
    const response = await probe.handle(new Request('http://localhost/v2/avatars/flags/us'))
  ...
  test('GET /avatars/credit-cards/:code → image/png', async () => {
    const response = await probe.handle(
      new Request('http://localhost/v2/avatars/credit-cards/visa'),
    )
    expect(response.status).toBe(200)
  })

  test('GET /avatars/browsers/:code → image/png', async () => {
    const response = await probe.handle(new Request('http://localhost/v2/avatars/browsers/chrome'))
    expect(response.status).toBe(200)
  })
  ...
  test('GET /avatars/qr requires text', async () => {
    const response = await probe.handle(new Request('http://localhost/v2/avatars/qr'))
