...
import { APP_POSTGRES_PASSWORD, AppMode, GET_PROJECT_DB, GET_PROJECT_DB_CLIENT, QueueFor } from "@nuvix/utils/constants";
...
export class AuditsQueue extends Queue implements OnModuleInit, OnModuleDestroy {
    ...
    private startTimer(): void {
        this.interval = setInterval(() => this.flushBuffer(), AuditsQueue.BATCH_INTERVAL_MS);
    }
    ...
    private async flushBuffer(): Promise<void> {
        ...
        for (const [projectId, data] of bufferCopy.entries()) {
            ...
            let client: Awaited<ReturnType<typeof this.getDatabase>>['client'] | undefined;
            try {
                ...
                this.logger.log(`Flushing ${logs.length} audit logs for project ${projectId}`);
                await audits.logBatch(logs);

            } catch (error) {
                this.logger.error(`Error flushing audit logs for project ${projectId}:`, error);
                // Re-add failed logs to buffer for retry
                const currentBuffer = this.buffer.get(projectId) || { project: data.project, logs: [] };
